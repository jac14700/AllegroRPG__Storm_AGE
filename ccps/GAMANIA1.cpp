...
int GAMANIA1(ALLEGRO_DISPLAY * display)
{
    ...
    /// 變數宣告區
    bool done =false,draw =true,Game=true;
    ...
    for(i=0;i<PLAYER_NUM;i++)
    {
        ...
        player[i].On_Ballx=0;
        player[i].On_Bally=0;
        player[i].revive_time_counter=ReviveTime;
    }
        player[0].x=775;
        player[0].y=675;
        player[1].x=75;
    ...
    /*    al_set_new_display_flags(ALLEGRO_OPENGL);
        ALLEGRO_DISPLAY *display= al_create_display(SCREEN_W,SCREEN_H);//主畫面的大小設置
        al_set_window_position(display,280,0);//window位置的放置
    ...
    */
                        ...
                        }if(al_key_down(&keyState,ALLEGRO_KEY_Q))//PLAYER2
                        {
                            ...
                            for(i=11;i<20;i++)
                            {
                                ...
                                case ALLEGRO_EVENT_TIMER:
                                {
                                    ...
                                    if(p1die)
                                    {printf("timer:%d",player[0].revive_time_counter);
                                        player[0].revive_time_counter--;
                                        if(player[0].revive_time_counter==0)
                                        {
                                            p1die=false;
                                            player[0].x=775;
                                            player[0].y=650;
                                            player[0].revive_time_counter=ReviveTime;
                                        }
                                    }if(p2die)
                                    {
                                        player[1].revive_time_counter--;
                                        if(player[1].revive_time_counter==0)
                                        {
                                            p2die=false;
                                            player[1].x=75;
                                            player[1].y=75;
                                            player[1].revive_time_counter=ReviveTime;
                                        }
                                    }
                                    if (++r >= 255) r = 0;
                                    if (++g >= 255) g = 0;
                                    ...
                                    {
                                        al_play_sample(starburst, 2, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
                    ...
                    for(i=0;i<MAX_BALL;++ i)
                    {
                        ...
                        {printf("%d\n",i);
                            ...
                            if(ball[i].countDown<0)
                            {
                                    ...
                                    if(i>=10&&i<20)
                                    ball_killer(ball[i].x,ball[i].y,Red,player[0].x,player[0].y,player[1].x,player[1].y,&p1die,&player[1].suicide,&p2die,&player[1].kill,player[1].power
    ...
    /////////////////////////////////////
                     ...
                     //al_draw_rectangle(player[1].x,player[1].y,player[1].x,player[1].y, Green,8.0);
                        ...
                        al_draw_textf(fonte1, al_map_rgb(255, 255, 255),973,163, ALLEGRO_ALIGN_CENTRE, "kill:%-2d",  player[0].kill);
                        al_draw_textf(fonte1, al_map_rgb(255,255,255),973,182, ALLEGRO_ALIGN_CENTRE, "suicide:%-2d", player[0].suicide);
                        al_draw_textf(fonte1, al_map_rgb(255, 255, 255),973,207, ALLEGRO_ALIGN_CENTRE, "kill:%-2d",  player[1].kill);
                        al_draw_textf(fonte1, al_map_rgb(255,255,255),973,227, ALLEGRO_ALIGN_CENTRE, "suicide:%-2d", player[1].suicide);
   ...
   if(!Game)
{
                                if(player[0].kill-2*player[0].suicide>player[1].kill-2*player[1].suicide)
